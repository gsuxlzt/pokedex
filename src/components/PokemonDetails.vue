<template>
    <div v-if="hasData">
        <div v-if="hasErrors" class="error-container">
                ...
                <h4 class="error-message">The Pokémon you are looking for cannot be found.</h4>
        </div>
        <div v-else class="details-container">
            <div class="general-details">
                <h1> {{ capitalizeName(details.name) }}</h1>
                <div class="type-cards">
                    <span v-for="type in details.types" :key="type.type.name" class="type" :style="{'--bg-color': getColorFromType(type.type.name)}">
                        {{ type.type.name }}
                    </span>
                </div>
            ...
            <div class="detail-group">
                ...
                <div class="abilities-details">
                    <h4>Abilities</h4>
                    <div class="info" v-for="ability in details.abilities" :key="ability.slot">
            ...
            <div class="stat-details">
                <div class="stat" v-for="stats in statsDetails" :key="stats.name">
                    ...
                    <div class="stat-bar"  :style="{'--bar-width': `${(stats.value/totalStats)*100}%`}">
                    </div>
                </div>
            </div>
            <div class="typing-details" v-if="hasType">
                ...
                <p>Weak against:</p>
                <div class="type-cards">
                    <span v-for="type in weakAgainst()" :key="type" class="type" :style="{'--bg-color': getColorFromType(type)}">
                        {{ type }}
                    </span>
                </div>
                <p>Strong against:</p>
                <div class="type-cards">
                    <span v-for="type in strongAgainst()" :key="type"  class="type" :style="{'--bg-color': getColorFromType(type)}">
                        {{ type }}
                    </span>
                </div>
            </div>
...
export default {
    ...
    methods: {
            ...
            try {
                ...
                this.spriteChange = setInterval(() => {
                    ...
                    if (!this.details.sprites[obj[this.spriteToShow]])
                        ...
                        this.spriteToShow = obj[this.spriteToShow];
                }, 1200)
            } catch(e) {
                this.hasErrors = true;
            }
        ...
        getFlavorText() {
            return this.details.flavor_text_entries.find(f => f.language.name === 'en').flavor_text;
        },
        async getPokemonTyping() {
            this.typing = {};
            await this.details.types.forEach(async type => {
                const { data: { damage_relations} } = await this.$http(type.type.url);
                Object.entries(damage_relations).forEach(([key, value]) => {
                    if (this.typing[key]) {
                        this.typing[key].push(...value);
                    } else
                        this.typing[key] = value;
                })
                this.hasType = true;
            });
        },
        ...
        sizeDetails() {
            ...
            Object.entries(obj).forEach(([key, val]) => {
                if (val && Object.prototype.hasOwnProperty.call(val, 'name'))
                    ...
                    arr.push({name: key, value: key === 'weight' ? `${val/10}kg` : `${val/10}m`});
                }
            })
...
<style lang="scss">
    ...
    .details-container {
        ...
        .detail-group {
            ...
            .size-details, .abilities-details {
                padding: 12px;
                margin: 8px 0;
